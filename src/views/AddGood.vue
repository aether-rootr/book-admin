  ...
  <div class="add">
    <el-card class="add-container">
      <el-form :model="goodForm" :rules="rules" ref="goodRef" label-width="100px" class="goodForm">
        ...
        <el-form-item label="商品作者" prop="goodsAuthor">
          <el-input style="width: 300px" v-model="goodForm.goodsAuthor" placeholder="请输入作者"></el-input>
        </el-form-item>
        <el-form-item label="商品出版社" prop="goodsPress">
          <el-input style="width: 300px" v-model="goodForm.goodsPress" placeholder="请输入出版社"></el-input>
        </el-form-item>
  ...
  setup() {
    ...
    const state = reactive({
      ...
      rules: {
        ...
        goodsAuthor: [
          { required: 'true', message: '请填写作者名称', trigger: ['change'] }
        ],
        goodsPress: [
          { required: 'true', message: '请填写出版社名称', trigger: ['change'] }
        ],
        originalPrice: [
      ...
      if (id) {
        axios.get(`/goods/${id}`).then(res => {
          ...
          state.goodForm = {
            ...
            goodsAuthor: goods.goodsAuthor,
            goodsPress: goods.goodsPress,
          ...
          console.log('state.defaultCate'+state.defaultCate)
          if (instance) {
            // 初始化商品详情 html
      ...
      goodRef.value.validate((vaild) => {
        if (vaild) {
          ...
          let params = {
            ...
            goodsAuthor: state.goodForm.goodsAuthor,
            goodsPress: state.goodForm.goodsPress,
          ...
          if (params.goodsAuthor.length > 40) {
            ElMessage.error('作者名称不能超过40个字符')
            return
          }
          if (params.goodsPress.length > 128) {
            ElMessage.error('出版社名称不能超过128个字符')
            return
          }
